...
from streammat.models import DataType, StreamMatConfig, StreamMatException, ErrorCode
...
def setup_and_teardown_tiledb():
    ...
    with tiledb.open(TEST_MATRIX_URI, 'w') as A:
        ...
        config = StreamMatConfig(rows=rows, cols=cols, nnz=nnz, dtype=DataType.FLOAT64)
...
@pytest.mark.asyncio
async def test_matvec(setup_and_teardown_tiledb, sparse_matrix_numpy):
    ctx = tiledb.Ctx()
...
@pytest.mark.asyncio
async def test_rmatvec(setup_and_teardown_tiledb, sparse_matrix_numpy):
    ctx = tiledb.Ctx()
    matrix = StreamMatrix(uri=TEST_MATRIX_URI, ctx=ctx)
...
@pytest.mark.asyncio
async def test_matvec_chunked(setup_and_teardown_tiledb, sparse_matrix_numpy):
    ctx = tiledb.Ctx()
    matrix = StreamMatrix(uri=TEST_MATRIX_URI, ctx=ctx)
    vector = np.array([1, 2, 3, 4], dtype=np.float64)
...
@pytest.mark.asyncio
async def test_rmatvec_chunked(setup_and_teardown_tiledb, sparse_matrix_numpy):
    ctx = tiledb.Ctx()
    matrix = StreamMatrix(uri=TEST_MATRIX_URI, ctx=ctx)
    vector = np.array([10, 20, 30, 40], dtype=np.float64)
...
@pytest.mark.asyncio
async def test_matvec_dimension_mismatch(setup_and_teardown_tiledb):
    ctx = tiledb.Ctx()
    matrix = StreamMatrix(uri=TEST_MATRIX_URI, ctx=ctx)
...
@pytest.mark.asyncio
async def test_rmatvec_dimension_mismatch(setup_and_teardown_tiledb):
    ctx = tiledb.Ctx()
    matrix = StreamMatrix(uri=TEST_MATRIX_URI, ctx=ctx)
