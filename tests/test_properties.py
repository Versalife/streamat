...
def sparse_matrices(draw):
    ...
    data = draw(arrays(dtype=dtype, shape=nnz, elements=floats(min_value=-1e3, max_value=1e3)))
...
def test_property_matvec(matrix_coo):
...
@settings(deadline=1000, suppress_health_check=[HealthCheck.function_scoped_fixture])
@given(matrix_coo=sparse_matrices())
def test_property_rmatvec(matrix_coo):
