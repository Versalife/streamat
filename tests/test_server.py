...
def create_test_tiledb_array(uri: str, dtype: np.dtype, lib_dtype: DataType) -> None:
    ...
    with tiledb.open(uri, "w") as A:
        ...
        config = StreamMatConfig(rows=rows, cols=cols, nnz=nnz, dtype=lib_dtype, version=1)
...
def setup_and_teardown_tiledb() -> Generator[None, None, None]:
    ...
    create_test_tiledb_array(TEST_MATRIX_URI, np.dtype(np.float64), DataType.FLOAT64)
    create_test_tiledb_array(TEST_MATRIX_FLOAT32_URI, np.dtype(np.float32), DataType.FLOAT32)
    yield
    # Teardown
...
def test_load_matrix(client: TestClient) -> None:
    matrix_name = "test_matrix_load"
    with client.stream(
        "PUT", f"/api/v1/matrix/{matrix_name}", json={"uri": TEST_MATRIX_URI}
    ) as response:
...
def test_matvec_success(client: TestClient) -> None:
    # Ensure matrix is loaded
    client.put("/api/v1/matrix/test_matrix", json={"uri": TEST_MATRIX_URI})

    response = client.post("/api/v1/matvec/test_matrix", json={"vector": [1, 2, 3, 4]})
    assert response.status_code == 200
...
def test_rmatvec_success(client: TestClient) -> None:
    # Ensure matrix is loaded
    client.put("/api/v1/matrix/test_matrix", json={"uri": TEST_MATRIX_URI})

    response = client.post(
        "/api/v1/rmatvec/test_matrix", json={"vector": [10, 20, 30, 40]}
    )
    assert response.status_code == 200
    result = response.json()["result"]
    # Expected: A.T * y = [1.1*10, 2.2*20, 3.3*30, 5.5*10 + 4.4*40] = [11.0, 44.0, 99.0,
    # 231.0]
    np.testing.assert_allclose(result, [11.0, 44.0, 99.0, 231.0])


def test_matvec_dtype_mismatch_fix(client: TestClient) -> None:
    ...
    response = client.post(
        "/api/v1/matvec/test_matrix_f32", json={"vector": [1, 2, 3, 4]}
    )
    assert response.status_code == 200
    ...
    )  # JSON conversion might make it float64
    np.testing.assert_allclose(result, [23.1, 4.4, 9.9, 17.6], rtol=1e-5)


def test_matrix_not_found(client: TestClient) -> None:
    response = client.post(
        "/api/v1/matvec/non_existent_matrix", json={"vector": [1, 2, 3, 4]}
    )
    assert response.status_code == 404
    assert response.json()["error"]["code"] == "MATRIX_NOT_FOUND"


def test_dimension_mismatch(client: TestClient) -> None:
    client.put("/api/v1/matrix/test_matrix", json={"uri": TEST_MATRIX_URI})
    response = client.post(
...
def test_status_endpoint_for_shape_discovery(client: TestClient) -> None:
...
def test_matvec_with_sparse_vector(client: TestClient) -> None:
    ...
    response = client.post("/api/v1/matvec/test_matrix", json={"vector": sparse_vector})
    assert response.status_code == 200
    result = response.json()["result"]
...
def test_rmatvec_with_sparse_vector(client: TestClient) -> None:
    ...
    response = client.post(
        "/api/v1/rmatvec/test_matrix", json={"vector": sparse_vector}
    )
    assert response.status_code == 200
    result = response.json()["result"]
...
def test_unload_matrix(client: TestClient) -> None:
    ...
    create_test_tiledb_array(unload_uri, np.dtype(np.float64), DataType.FLOAT64)
    assert os.path.exists(unload_uri)

    # First, load the matrix
    with client.stream(
        "PUT", f"/api/v1/matrix/{matrix_name}", json={"uri": unload_uri}
    ) as response:
        for line in response.iter_lines():
            if "loading_complete" in line:
                break
...
def test_matmul_success(client: TestClient) -> None:
    ...
    try:
        response = client.post(
            "/api/v1/matmul/matrix_a/matrix_b", json={"output_uri": output_uri}
        )
        assert response.status_code == 200
        assert response.json() == {
            "message": (
                f"Matrix multiplication complete. Result stored at '{output_uri}'."
            )
        }
