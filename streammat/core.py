...
class StreamMatrix:
    ...
    def __init__(self, uri: str, ctx: tiledb.Ctx) -> None:
        ...
        try:
            ...
            logger.success(
                f"Successfully validated and loaded metadata for '{self.uri}'."
            )
    ...
    def _load_and_validate_meta(self) -> None:
        ...
        dtype_map: dict[DataType, type[np.number]] = {
    ...
    async def matvec(self, vector: np.ndarray, chunk_size: int = 1000) -> np.ndarray:
        """
        ...
        if vector.shape != (self.config.cols,):
            raise StreamMatException(
                ...
                f"Input vector has shape {vector.shape}, but matrix has "
                f"{self.config.cols} columns.",
            )
        ...
        logger.info(
            f"Starting matvec operation for {self.config.rows} rows with chunk size {chunk_size}."
        )
        ...
        for r_start, chunk_result in zip(
            range(0, self.config.rows, chunk_size), chunk_results
        ):
    ...
    async def _get_row_chunk_dot_product(
        self, r_start: int, r_end: int, vector: np.ndarray
    ) -> np.ndarray:
        ...
        scipy_sparse_matrix = coo_matrix(
            (values, (row_indices - r_start, col_indices)),
            shape=(r_end - r_start, self.config.cols),
        )

        return np.asarray(scipy_sparse_matrix.dot(vector))

    async def rmatvec(self, vector: np.ndarray, chunk_size: int = 1000) -> np.ndarray:
        """
        ...
        if vector.shape != (self.config.rows,):
            raise StreamMatException(
                ErrorCode.DIMENSION_MISMATCH,
                f"Input vector has shape {vector.shape}, but matrix has "
                f"{self.config.rows} rows.",
            )
        if self.config.nnz == 0:
            logger.warning(
                "Performing rmatvec on a matrix with zero non-zero elements."
            )
            return np.zeros(self.config.cols, dtype=self._numpy_dtype)

        logger.info(
            f"Starting rmatvec operation for {self.config.cols} columns with chunk size {chunk_size}."
        )
        ...
        for c_start, chunk_result in zip(
            range(0, self.config.cols, chunk_size), chunk_results
        ):
    ...
    async def _get_col_chunk_dot_product(
        self, c_start: int, c_end: int, vector: np.ndarray
    ) -> np.ndarray:
        ...
        scipy_sparse_matrix = coo_matrix(
            (values, (row_indices, col_indices - c_start)),
            shape=(self.config.rows, c_end - c_start),
        )

        return np.asarray(scipy_sparse_matrix.T.dot(vector))

    async def matmul(self, other: Self, output_uri: str, chunk_size: int = 100) -> None:
        """
        ...
        if self.config.cols != other.config.rows:
            raise StreamMatException(
                ErrorCode.DIMENSION_MISMATCH,
                f"Matrix A has {self.config.cols} columns, but matrix B has "
                f"{other.config.rows} rows.",
            )

        logger.info(
            f"Starting matmul operation: {self.uri} * {other.uri} -> {output_uri}"
        )
        ...
        domain = tiledb.Domain(
            tiledb.Dim(
                name=DIM_ROW_NAME,
                domain=(0, output_rows - 1),
                tile=min(output_rows, 4096),
                dtype=np.uint64,
            ),
            tiledb.Dim(
                name=DIM_COL_NAME,
                domain=(0, output_cols - 1),
                tile=min(output_cols, 4096),
                dtype=np.uint64,
            ),
        )
        ...
        for c_start in range(0, other.config.cols, chunk_size):
            ...
            try:
                ...
                logger.exception(
                    f"An error occurred during matmul for matrix {self.uri}"
                )
                raise StreamMatException(
                    ErrorCode.INTERNAL_SERVER_ERROR,
    ...
    def _get_col(self, col_index: int) -> np.ndarray:
